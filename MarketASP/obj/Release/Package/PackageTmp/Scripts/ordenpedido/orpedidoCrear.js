        ...
        $.ajax({
            ...
            success: function (resultado) {
                ...
                mattable = $('#matetabla').DataTable({
                    ...
                    "aoColumnDefs": [{
                        "bVisible": false,
                        "aTargets": [0, 9, 10,11,12,13,15,16]
                    },
                    {
                        "sClass": "my_class",
                        "aTargets": []
...
/*lista de lotes*/
    ...
    $("#btnMatTablaLote").click(function () {
        ...
        $.ajax({
            ...
            //contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: { ncode_alma: ncodealma, ncode_arti: data.Cod, fvenci_lote: '', sdesc_lote: '' },
            success: function (resultado) {
                //console.log(resultado);

                if (resultado.length == 0) {

                    loteMatTable.clear().draw();
                    alert("No hay lotes disponibles para venta")

                    return;
                }

                if (typeof loteMatTable !== 'undefined') {
                    loteMatTable.destroy();
                }

                


                loteMatTable = $('#loteMatTabla').DataTable({
                    data: resultado, ///JSON.parse(data.d),
                    "columns":
                        [{ "data": "ncode_lote" },
                        { "data": "sdesc_lote" },
