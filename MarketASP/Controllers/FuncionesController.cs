...
namespace MarketASP.Controllers
{
    public class FuncionesController : Controller
    {
        ...
        /* Nueva función para traer la descripcion de acuerdo al ubigeo obtenida del API RUC*/
        public JsonResult getUbigeoNew(string scodeUbigeo)
        {           
            db.Configuration.ProxyCreationEnabled = false;
            var result = from s in db.UBIGEO
                         where s.scode_ubigeo == scodeUbigeo
                         select new { direccion = s.sdistri_ubigeo + " " + s.sprovi_ubigeo + " " + s.sdepa_ubigeo };
        ...
        {
            var resultado = db.Pr_KardexArticulos(ncode_arti,"","",0,0);
        ...
        public JsonResult getStockDisponible(int ncode_arti, int ncode_alma)
        {
            try
            {
                var resultado = db.Pr_KardexArticulos(ncode_arti, "", "", ncode_alma,0).ToList();
